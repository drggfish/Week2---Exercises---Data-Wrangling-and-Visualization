...
dat <- read.csv("../data/In_Class_Exercises_Arsenic.csv")

png("arsenic_boxplot.png", width = 1500, height = 1500, res = 300)
...
p <- ggplot(dat, aes(x = Gender, y = Arsenic_Total, fill = Marital_Status)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7, position = position_dodge(width = 0.75)) +
      geom_jitter(aes(color = Marital_Status), position = position_dodge(width = 0.75), size = 2, alpha = 0.6) + 
      labs(x = "Gender", y = "Arsenic_Total", fill = "Marital Status", color = "Marital Status", title = "Arsenic Total Distribution \nby Gender and Marital Status") +
  theme_minimal() +
